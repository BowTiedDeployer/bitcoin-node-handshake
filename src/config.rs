...
pub enum CliArguments {
    ...
    IPList((bool, Option<u32>, Vec<SocketAddr>)),
}
...
pub enum ConfigError {
    #[error(
        "Command not specified or invalid! Run 'cargo run -- -h' for more help with commands."
    )]
    InvalidCommand,
    #[error(
        "Invalid DNS list passed as argument! Run 'cargo run -- -h' for more help with commands."
    )]
    InvalidDNSList,
    #[error(
        "Invalid IP list passed as argument! Run 'cargo run -- -h' for more help with commands."
    )]
    InvalidIPList,
...
fn match_arguments(
    command: &CliCommands,
    mut cli_arguments: Args,
) -> Result<(CliArguments, CliNetwork), ConfigError> {
    ...
    match command {
        ...
        CliCommands::ListIPSCommand => {
            ...
            while let Some(arg) = cli_arguments.next() {
                match arg.as_str() {
                    ...
                    _ if DNS_LIST_MAINNET.contains(&arg.as_str())
                        || DNS_LIST_TESTNET.contains(&arg.as_str()) =>
                    {
                        dns_args.push(arg)
                    }
        ...
        CliCommands::HandshakeCommand => {
            ...
            if ip_args.is_empty() && number.is_none() {
                ...
                arguments = (
                    CliArguments::IPList((write_in_file, number, ip_args)),
                    network,
                );
            }
        }
...
pub async fn run(config: &Config) -> Result<(), ConfigError> {
    match &config.command {
        ...
        CliCommands::HandshakeCommand => {
            if let CliArguments::IPList((print_in_file, ips_number, ip_list)) = &config.args {
                let mut ip_list = ip_list.clone();

                if let Some(number_of_ips) = ips_number {
                    ip_list = get_ips_for_handshake(*number_of_ips, &config.network).await;
                }
...
fn print_help_menu() {
    ...
    println!(
        "        Display a list of a given number of IPs from each of the given DNS in the list."
    );
    println!("        Use the -t flag to run on testnet.\n");
