...
use crate::consts::{DNS_LIST_MAINNET, DNS_LIST_TESTNET, PORT_MAINNET, PORT_TESTNET};
use std::net::SocketAddr;
use tokio::net::lookup_host;

pub async fn get_ips_for_dns_list(
    args: &CliArguments,
    network: &CliNetwork,
...
pub async fn get_ips_for_handshake(number_of_ips: u32, network: &CliNetwork) -> Vec<SocketAddr> {
    ...
    let ip_number_from_each_dns =
        distribute_ip_number_for_each_dns(available_dns_seeds.len(), number_of_ips);

    for (position, &dns) in available_dns_seeds.iter().enumerate() {
        if ip_number_from_each_dns[position] != 0 {
            let ips =
                get_ips_for_given_dns(ip_number_from_each_dns[position], &dns.to_string(), network)
                    .await;
...
async fn get_ips_for_given_dns(
    number_of_ips_per_dns: u32,
    dns: &String,
    network: &CliNetwork,
) -> Vec<SocketAddr> {
    let mut ip_list = vec![];
